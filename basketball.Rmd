...
ggplot(data = nba_players, aes(x = raptor_defense, y = raptor_offense)) +
  ...
  labs(title = "Distrobution of NBA Players", subtitle = "By Raptor Offense and Raptor Defense")
...
ggplot(data = all_nba) + geom_bar(aes(player_name, raptor_total, fill = player_name, xaxt=FALSE), stat = "identity")+theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank()) +
...
#remove x axis values, not label 
...
Graph 3: NEED TO EXPLAIN BC I DONT GET WAR THAT MUCH. Interesting things:
Chris Paul had a negative total raptor but is very high WAR. 
No negative WARs
Russell with low WAR and neg raptor
...
team_data <- nba_players %>%
  select(team, team_win_pc, raptor_total) %>%
  group_by(team) %>%
  summarise(mean(raptor_total))

#team_win_pct_data <- nba_players %>%
 # select(team, team_win_pc) %>%
  #group_by(team_win_pc)
