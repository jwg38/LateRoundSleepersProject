...
In terms of suggestions for improving our analysis, we might have rerun
our correlation tests multiple ways in order to ensure their success. In our first correlation test, the QQ plots showed some non-normality. We could have come up with more accurate
conclusions if the condition of normality was met. In addition, we performed a 
T-test on the full population. Due to the fact that NBA changes every year, 
performing a T-test on the full population might suggest accurate predictions 
about future years. In terms of our data, most of the variables are numeric, and do not necessarily contain all of the nuances of the NBA, such as 
coaching, team morale, synergy between teammates, and injuries.

If we were to redo this assignment, we would have chosen a similar data set but one with 
...
have chosen a data set that included simpler statistics as well, such as points per game, 
rebounds per game, and assists per game, in order to analyze the change of these 
stats over time. The NBA has become much more of an offense-based league, and it 
has been interesting to observe this change analytically. If we were to 
continue work on this project, we would try to find linear models that fit 
